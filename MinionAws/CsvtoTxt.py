# This file contains the 'convert(IMEI)' function. This takes the imei of a csv in \csv and converts the 
# hex paylaods into readable ascii. This is saved into a 'imei_xxx.txt' file in the \txt folder. 
# $ at the begining of a line indicate a new file.  The 'create_files(imei)' function takes this 
# main txt file and generates each indivual file while adding time data in unix time. The create_files function
# will automatically call the convert function. 

# Run this program for a given IMEI only once all data for the given device has been recived from the queue sever. 
# For this reson if a txt directory for a given device exists the program will exit. If the program was run before 
# all data was recived, simply delete the txt_xxxx$ folder where the xxxx's are the IMEI of the device. Then move the 
# csv imei_xxxx.csv from the \\csv_processed folder to the \\csv folder. Then run MOmsg to append the rest of the queue msgs appended 
# to the csv. 
...
# is expected to be a csv in \\csv generated by MOmsg. This is used to generate the 
# indvidual files, then the maintxt file is deleted once it is no longer needed. 
...
# into multiple text files each containg one complete file. This function is whitespace sensitive. 
# so the text file must have no blank lines. 
def create_files(IMEI, dir): 
    ...
    # Now that all text files have been made we need to move the CSV out of \\csv so that new minion runs can be 
    # preformed. If the csv file is left in \cvs and the minion is re deployed it will append the new data to the old 
    # csv. 
   ## try: 
   ##     os.mkdir(os.getcwd()+'\\csv_processed')
   ## except: 
   ##     print('csv_processed already exists')
   ## os.rename(os.getcwd() + '\\csv\\imei_{}.csv'.format(IMEI), os.getcwd()+ '\\csv_processed\\imei_{}.csv'.format(IMEI))
